...
//            }
//        }
        // Удаляем из списка ожидания (используется remove(Object))
        waitingList.remove(Integer.valueOf(studentId));
        // Удаляем из основного списка (используется remove(Object))
        boolean wasRemoved = enrollList.remove(Integer.valueOf(studentId));

        if (wasRemoved && hasWaitingList()) {
            enrollList.add(waitingList.removeFirst());
        }
    }
